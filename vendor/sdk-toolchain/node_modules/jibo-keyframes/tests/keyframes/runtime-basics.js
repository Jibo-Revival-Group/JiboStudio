...
function validateDefaults(newKeyframes) {
    ...
    assert.equal(newKeyframes.version, "0.15.0", "Incorrect default version");
    assert.equal(newKeyframes.layers.length, 3, "Incorrect number of default layers");

    assert.equal(newKeyframes.layers[0].name, "Eye", "Incorrect layer name");
    assert.equal(JiboKeyframeInfo.layerTypes[newKeyframes.layers[0].type], JiboKeyframeInfo.layerTypes.Eye, "Incorrect layer type class");

    assert.equal(newKeyframes.layers[0].visible, true, "Incorrect layer visibility");
    assert.equal(newKeyframes.layers[0].locked, false, "Incorrect layer locked value");
}
